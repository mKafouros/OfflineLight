...
class FixedTimeAgent(BaseAgent):
    ...
    """
    def __init__(self, action_space, I, world, ob_generator=None, rw_generator=None):
        super().__init__(action_space)
        self.I = I
        self.world = world
        self.world.subscribe("lane_count")
        self.ob_generator = ob_generator
        self.rw_generator = rw_generator
    ...
    def get_reward(self):
        if self.rw_generator is None:
            return None
        else:
            reward = self.rw_generator.generate()
            assert len(reward) == 1
            return reward[0]