...
class DQNAgent(RLAgent):
    def __init__(self, action_space, ob_generator, reward_generator, iid):
        ...
        self.memory_offline = deque(maxlen=2000)
        self.memory_mb = deque(maxlen=200)
    ...
    def get_action(self, ob, test=True):
    ...
    def remember_offline(self, ob, action, reward, next_ob):
        self.memory.append((ob, action, reward, next_ob))

    def remember_model(self, ob, action, reward, next_ob):
        self.memory.append((ob, action, reward, next_ob))
    ...
    def replay(self):
        ...
        t1 = time.time()
        # print("replay time: {}".format(t1-t0))

    def load_model(self, dir="saves/model/dqn/", model_id=None):
        if model_id is not None:
            name = "dqn_agent_{}_iter_{}.h5".format(self.iid, model_id)
        else:
            name = "dqn_agent_{}.h5".format(self.iid)
        model_name = os.path.join(dir, name)
        self.model.load_weights(model_name)

    def save_model(self, dir="saves/model/dqn/", model_id=None):
        if model_id is not None:
            name = "dqn_agent_{}_iter_{}.h5".format(self.iid, model_id)
        else:
            name = "dqn_agent_{}.h5".format(self.iid)
